...
#### 3.2 最優先: 基本CLIインターフェース 🚀
**目標**: ユーザーがコマンドを打って確認できる状態

- [ ] **Cobraコマンド統合** ⭐ 最重要
  - [ ] cmd/root.goでのフラグ定義
  - [ ] git.ExecuteCleanupとの統合
  - [ ] テスト: 基本コマンド実行
...
- **最優先実装**: Phase 3.2 基本CLIインターフェース

### Phase 3.2 実装戦略（ユーザー体験最優先）

#### 🎯 目標
**ユーザーがコマンドラインでgitcを実行し、結果を確認できる状態**

#### 📋 実装順序
1. **Cobraコマンド統合**（最重要）
   - cmd/root.goでのフラグ定義とコマンド実行
   - internal/git.ExecuteCleanupとの統合
   
2. **安全確認機能**
   - `--dry-run`フラグ実装（実際の変更なし）
   - ドライランモードでの結果表示
   
3. **基本フラグ**
   - `--version`, `--help`, `--yes`の実装
