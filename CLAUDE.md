...
#### 3.3 最優先: Fetch/Pull機能の追加 🔄
**目標**: デフォルトブランチで最新の状態を取得してからクリーンアップ

- [ ] **Fetch機能の実装** ⭐ 最重要
  - [ ] `git fetch --all --prune` の実装
  - [ ] internal/git/remote.goのFetch関数を活用
  - [ ] テスト: fetch処理の動作確認
...
- [ ] **実行順序の確保**
  1. デフォルトブランチへ切り替え
  2. `git fetch --all --prune` 実行
  3. `git pull` 実行
  4. 不要なブランチ削除
  
- [ ] **フラグ追加**
  - [ ] `--no-fetch`: fetch処理をスキップ
  - [ ] `--no-pull`: pull処理をスキップ（既存実装の活用）
  - [ ] テスト: 各フラグの動作確認
